...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None,dfeat_db=None,sfeat_db=None, batch_size=100):
        """
        ...
        self.dfeat_db = dfeat_db
        self.sfeat_db = sfeat_db
        self.image_w = 640
        self.image_h = 480
        self.vfov = 60
    ...
    def getStates(self):
        ...
        """
        feature_depth_sem_states = []
        for i, sim in enumerate(self.sims):
            state = sim.getState()[0]

            feature = self.feat_db.get_image_feature(state.scanId, state.location.viewpointId)
            dfeature = self.dfeat_db.get_image_feature(state.scanId, state.location.viewpointId)
            sfeature = self.sfeat_db.get_image_feature(state.scanId, state.location.viewpointId)

            feature_depth_sem_states.append((feature,dfeature,sfeature, state))

        return feature_depth_sem_states
...
class ReverieObjectNavBatch(object):
    ...
    def __init__(
        self, view_db, dview_db, sview_db, obj_db, instr_data, connectivity_dir, obj2vps,
        ...
        self.env = EnvBatch(connectivity_dir, feat_db=view_db, dfeat_db=dview_db, sfeat_db=sview_db, batch_size=batch_size)
        self.obj_db = obj_db
    ...
    def make_candidate(self, feature, dfeature, sfeature, scanId, viewpointId, viewId):
        # todo: add the depth feature
        ...
        if long_id not in self.buffered_state_dict:
            for ix in range(36):
                ...
                depth_feat = dfeature[ix]
                sem_feat = sfeature[ix]
                ...
                for j, loc in enumerate(state.navigableLocations[1:]):
                    ...
                    if (loc.viewpointId not in adj_dict or
                        ...
                        adj_dict[loc.viewpointId] = {
                            ...
                            'dfeature': np.concatenate((depth_feat, angle_feat), -1),
                            'sfeature': np.concatenate((sem_feat, angle_feat), -1),
            ...
            for c in candidate:
                ...
                c_new['dfeature'] = np.concatenate((depth_feat, angle_feat), -1)
                c_new['sfeature'] = np.concatenate((sem_feat, angle_feat), -1)
    ...
    def _get_obs(self):
        obs = []
        for i, (feature, dfeature, sfeature, state) in enumerate(self.env.getStates()):
            item = self.batch[i]
            base_view_id = state.viewIndex

            # Full features
            candidate = self.make_candidate(feature, dfeature, sfeature, state.scanId, state.location.viewpointId, state.viewIndex)
            # [visual_feature, angle_feature] for views
            feature = np.concatenate((feature, self.angle_feature[base_view_id]), -1)
            dfeature = np.concatenate((dfeature, self.angle_feature[base_view_id]), -1)
            sfeature = np.concatenate((sfeature, self.angle_feature[base_view_id]), -1)
    ...
    ############### Nav Evaluation ###############
    def _eval_item(self, scan, pred_path, predObjId, gt_path, gt_objid):
        scores = {}
        ...
        scores['rgs'] = str(predObjId) == str(gt_objid)
    ...
    def eval_metrics(self, preds):
        ...
        for item in preds:
            ...
            predObjId = item.get('predObjId', None)
            scan, gt_traj, gt_objid = self.gt_trajs[instr_id]
            traj_scores = self._eval_item(scan, traj, predObjId, gt_traj, gt_objid)
