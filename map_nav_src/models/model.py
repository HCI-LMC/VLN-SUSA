...
class VLNBert(nn.Module):
    def __init__(self, args):
        ...
        self.depth_linear = nn.Linear(1000, 768)
    ...
    def forward(self, mode, batch):
        ...
        if mode == 'language':
            ...
            # print(batch['view_img_fts'])
            batch['view_img_fts'] = self.drop_env(batch['view_img_fts'])
            batch['view_sem_fts'] = self.drop_env(batch['view_sem_fts'])
            if 'obj_img_fts' in batch:
                batch['obj_img_fts'] = self.drop_env(batch['obj_img_fts'])
            pano_sem_embeds, pano_sem_masks = self.vln_bert(mode, batch)
            return pano_sem_embeds, pano_sem_masks

        elif mode == 'depth_panorama':
            batch['view_depth_fts'] = self.depth_linear(batch['view_depth_fts'])
            batch['view_img_fts'] = self.drop_env(batch['view_img_fts'])
            if 'obj_img_fts' in batch:
                batch['obj_img_fts'] = self.drop_env(batch['obj_img_fts'])
            pano_depth_embeds, pano_depth_masks = self.vln_bert(mode, batch)
            return pano_depth_embeds, pano_depth_masks
