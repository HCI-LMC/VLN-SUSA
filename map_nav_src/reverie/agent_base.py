...
class BaseAgent(object):
    ...
    def get_results(self, detailed_output=False):
        ...
        for k, v in self.results.items():
            if v['predObjId'] is None:
                output.append({'instr_id': k, 'trajectory': v['path'], 'predObjId': v['predObjId']})
            else:
                output.append({'instr_id': k, 'trajectory': v['path'], 'predObjId': int(v['predObjId'])})
            if detailed_output:
