...
from utils.data import ImageFeaturesDB, DepthFeaturesDB, SemFeaturesDB
...
def build_dataset(args, rank=0):
    ...
    dfeat_db = DepthFeaturesDB(args.depth_ft_file)
    sfeat_db = SemFeaturesDB(args.sem_ft_file)
    ...
    if args.aug is not None:
        ...
        aug_env = dataset_class(
            feat_db, dfeat_db, sfeat_db, obj_db, aug_instr_data, args.connectivity_dir, obj2vps,
    ...
    if args.aug_only:
        ...
        train_env = dataset_class(
            feat_db, dfeat_db, sfeat_db,obj_db, train_instr_data, args.connectivity_dir, obj2vps,
            batch_size=args.batch_size, max_objects=args.max_objects,
    ...
    for split in val_env_names:
        ...
        val_env = dataset_class(
            feat_db, dfeat_db, sfeat_db,obj_db, val_instr_data, args.connectivity_dir, obj2vps, batch_size=args.batch_size,
            angle_feat_size=args.angle_feat_size, seed=args.seed + rank,
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    for idx in range(start_iter, start_iter + args.iters, args.log_every):
        ...
        if default_gpu:
            ...
            NEG_loss = 0.
            # NEG_loss = sum(listner.logs['NEG_loss']) / max(len(listner.logs['NEG_loss']), 1)
            entropy = sum(listner.logs['entropy']) / total
            writer.add_scalar("loss/critic", critic_loss, idx)
            writer.add_scalar("policy_entropy", entropy, idx)
            writer.add_scalar("loss/OG_loss", OG_loss, idx)
            writer.add_scalar("loss/IL_loss", IL_loss, idx)
            # writer.add_scalar("loss/NEG_loss", NEG_loss, idx)
            writer.add_scalar("total_actions", total, idx)
            writer.add_scalar("max_length", length, idx)
            write_to_record_file(
                "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, OG_loss %.4f, NEG_loss %.4f, policy_loss %.4f, critic_loss %.4f" % (
                    total, length, entropy, IL_loss, OG_loss, NEG_loss, policy_loss, critic_loss),
                record_file
            )
