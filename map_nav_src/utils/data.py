...
class DepthFeaturesDB(object):
    def __init__(self, depth_ft_file):
        self.depth_ft_file = depth_ft_file
        self._dfeature_store = {}

    def get_image_feature(self, scan, viewpoint):
        key = '%s_%s' % (scan, viewpoint)

        if key in self._dfeature_store:
            dft = self._dfeature_store[key]
        else:
            with h5py.File(self.depth_ft_file, 'r') as f:
                dft = f[key][...][:, :1000].astype(np.float32)
                self._dfeature_store[key] = dft

        return dft.squeeze(2)

class SemFeaturesDB(object):
    def __init__(self, sem_ft_file):
        self.sem_ft_file = sem_ft_file
        self._sfeature_store = {}

    def get_image_feature(self, scan, viewpoint):
        key = '%s_%s' % (scan, viewpoint)

        if key in self._sfeature_store:
            sft = self._sfeature_store[key]
        else:
            with h5py.File(self.sem_ft_file, 'r') as f:
                sft = f[key][...][:, :768].astype(np.float32)
                self._sfeature_store[key] = sft

        return sft.squeeze(2)
