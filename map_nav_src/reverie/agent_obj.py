...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _panorama_feature_variable(self, obs):
        ...
        batch_view_img_fts, batch_view_depth_fts, batch_view_sem_fts, batch_obj_img_fts, batch_loc_fts, batch_nav_types = [], [],[], [], [], []
        ...
        # print(batch_view_img_fts.size(),batch_view_depth_fts.size())

        return {
            'view_img_fts': batch_view_img_fts, 'view_depth_fts': batch_view_depth_fts,
            'view_sem_fts': batch_view_sem_fts, 'obj_img_fts': batch_obj_img_fts,
    ...
    def _nav_vp_variable(self, obs, gmaps, pano_embeds, pano_depth_embeds, pano_sem_embeds, cand_vpids, view_lens, obj_lens, nav_types):
        batch_size = len(obs)
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        # build depth graph: keep the start viewpoint
        gdmaps = gmaps
        gsmaps = gmaps
        ...
        txt_embeds = self.vln_bert('language', language_inputs)  # here
        ...
        for t in range(self.args.max_action_len):
            ...
            pano_depth_embeds, pano_depth_masks = self.vln_bert('depth_panorama', pano_inputs)  # here 补充view的view_depth_fts
            ...
            nav_outs = self.vln_bert('navigation', nav_inputs)  # here-gmap_img_embeds,gmap_depth_embeds
            ...
            if train_ml is not None:
                ...
                ''' 
                neg_loss += logits_neg[torch.arange(logits_neg.size(0)), a].mean()
                mll_loss += self.criterion(nav_logits, nav_targets)
                ml_loss = mll_loss - neg_loss
                '''
                ...
                # fused_outputs = gmap_outputs + vp_outputs + gmap_depth_outputs + vp_sem_outputs # better
            ...
            for i in range(batch_size):
                if (not ended[i]) and just_ended[i]:
                    ...
                    traj[i]['predObjId'] = stop_score['og']
        ...
        if train_ml is not None:
            ...
            # self.logs['NEG_loss'].append(neg_loss.item())
