...
class ImageEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        # self.depth_linear = nn.Linear(config.depth_feat_size, config.hidden_size)
...
class ImageSemEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ClsPrediction2(nn.Module):
    def __init__(self, hidden_size, input_size=None):
        super().__init__()
        if input_size is None:
            input_size = hidden_size
...
class SemEncoder(nn.Module):
    ...
    def forward(self, vp_sem, ins_features):
        # w = nn.Parameter(torch.randn_like(vp_sem))
        # 跨注意力的local sem output
        attention_output, attention_scores = self.cross_attn(vp_sem, ins_features)
        # 相似度汇聚的local sem output
        # 计算余弦相似度，维度：[batch_size, num_views, num_words]
        similarity = F.cosine_similarity(vp_sem.unsqueeze(2), ins_features.unsqueeze(1), dim=-1)
        # 对每一行（即每个view）取最大值，维度：[batch_size, num_views]
        max_similarity, _ = torch.max(similarity, dim=-1)
        # 将相似度值与视觉特征的形状匹配，扩展最后一个维度为768
        similarity_output = max_similarity.unsqueeze(-1).expand(-1, -1, 768)
        # adaptive = self.learnable_param
        w = 0.5 # 0, 0.2, 0.5, 0.8, 1.0， self.learnable_param
        sem_output = w * attention_output + (1.0 - w) * similarity_output
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.img_sem_embeddings = ImageSemEmbeddings(config)
        self.sem_encoder_head = SemEncoder(config)

        self.local_encoder = LocalVPEncoder(config)
        self.local_sem_encoder = LocalVPEncoder(config)
        self.global_encoder = GlobalMapEncoder(config)
        self.global_depth_encoder = GlobalMapEncoder(config)

        self.global_sap_head = ClsPrediction(self.config.hidden_size)
        self.global_depsap_head = ClsPrediction(self.config.hidden_size)
        self.local_sap_head = ClsPrediction(self.config.hidden_size)
        self.local_semsap_head = ClsPrediction(self.config.hidden_size)
        if config.glocal_fuse:
            self.sap_fuse_linear = ClsPrediction2(self.config.hidden_size, input_size=self.config.hidden_size * 4)
        else:
    ...
    def forward_sem_panorama_per_step(
            self, view_semsq_fts, obj_img_fts, loc_fts, nav_types, view_lens,obj_lens
    ):
        device = view_semsq_fts.device
        has_obj = obj_img_fts is not None

        view_sem_embeds = self.img_sem_embeddings.img_layer_norm(
            self.img_sem_embeddings.img_linear(view_semsq_fts)
        )
        if has_obj:
            if self.img_sem_embeddings.obj_linear is None:
                obj_img_embeds = self.img_sem_embeddings.img_layer_norm(
                    self.img_sem_embeddings.img_linear(obj_img_fts)
                )
            else:
                obj_img_embeds = self.img_sem_embeddings.obj_layer_norm(
                    self.img_sem_embeddings.obj_linear(obj_img_fts)
                )
            sem_embeds = []
            for view_embed, obj_embed, view_len, obj_len in zip(
                    view_sem_embeds, obj_img_embeds, view_lens, obj_lens
            ):
                if obj_len > 0:
                    sem_embeds.append(torch.cat([view_embed[:view_len], obj_embed[:obj_len]], 0))
                else:
                    sem_embeds.append(view_embed[:view_len])
            sem_embeds = pad_tensors_wgrad(sem_embeds)
            pano_lens = view_lens + obj_lens
        else:
            sem_embeds = view_sem_embeds
            pano_lens = view_lens

        pano_embeds = sem_embeds + \
                      self.img_sem_embeddings.loc_layer_norm(self.img_sem_embeddings.loc_linear(loc_fts)) + \
                      self.img_sem_embeddings.nav_type_embedding(nav_types) + \
                      self.embeddings.token_type_embeddings(torch.ones(1, 1).long().to(device))
        pano_embeds = self.img_sem_embeddings.layer_norm(pano_embeds)
        pano_embeds = self.img_sem_embeddings.dropout(pano_embeds)

        pano_masks = gen_seq_masks(pano_lens)
        if self.img_sem_embeddings.pano_encoder is not None:
            pano_embeds = self.img_sem_embeddings.pano_encoder(
                pano_embeds, src_key_padding_mask=pano_masks.logical_not()
            )
        return pano_embeds, pano_masks

    def forward_depth_panorama_per_step(
            self, view_img_fts, obj_img_fts, loc_fts, nav_types, view_lens, obj_lens
    ):
        device = view_img_fts.device
        has_obj = obj_img_fts is not None
    ...
    def forward_navigation_per_step(
            self, txt_embeds, txt_masks, gmap_img_embeds, gmap_depth_embeds, gmap_sem_embeds, gmap_step_ids, gmap_pos_fts,
            gmap_masks, gmap_pair_dists, gmap_visited_masks, gmap_vpids,
            vp_img_embeds, vp_depth_embeds, vp_sem_embeds, vp_pos_fts, vp_masks, vp_nav_masks, vp_obj_masks, vp_cand_vpids,
    ):
        ...
        # global2 branch
        gmap2_embeds = gmap_depth_embeds + \
                      self.global_depth_encoder.gmap_step_embeddings(gmap_step_ids) + \
                      self.global_depth_encoder.gmap_pos_embeddings(gmap_pos_fts)

        gmap2_embeds = self.global_depth_encoder.encoder(
            txt_embeds, txt_masks, gmap2_embeds, gmap_masks,
            graph_sprels=graph_sprels
        )
        ...
        # vp_sem_embeds = vp_sem_embeds + self.local_sem_encoder.vp_pos_embeddings(vp_pos_fts)
        # vp_sem_embeds = self.local_sem_encoder.encoder(txt_embeds, txt_masks, vp_sem_embeds, vp_masks)
        vp_sem_embeds = vp_sem_embeds + self.local_encoder.vp_pos_embeddings(vp_pos_fts)
        vp_sem_embeds = self.sem_encoder_head(vp_sem_embeds, txt_embeds)
        ...
        if self.sap_fuse_linear is None:
            ...
            fuse_weights = torch.sigmoid(self.sap_fuse_linear(
                torch.cat([gmap_embeds[:, 0], gmap2_embeds[:, 0], vp_embeds[:, 0], vp_sem_embeds[:, 0]], 1)
            ))
            fw = F.softmax(fuse_weights, 1)
        fw_global = fw[:, 0].unsqueeze(1)
        fw_global2 = fw[:, 1].unsqueeze(1)
        fw_local = fw[:, 2].unsqueeze(1)
        fw_local_sem = fw[:, 3].unsqueeze(1)

        global_logits = self.global_sap_head(gmap_embeds).squeeze(2) * fw_global
        ...
        global2_logits = self.global_depsap_head(gmap2_embeds).squeeze(2) * fw_global2
        global2_logits.masked_fill_(gmap_visited_masks, -float('inf'))
        global2_logits.masked_fill_(gmap_masks.logical_not(), -float('inf'))

        local_logits = self.local_sap_head(vp_embeds).squeeze(2) * fw_local
        local_logits.masked_fill_(vp_nav_masks.logical_not(), -float('inf'))
        # print('local', torch.softmax(local_logits, 1)[0], local_logits[0])
        local_logits_sem = self.local_semsap_head(vp_sem_embeds).squeeze(2) * fw_local_sem
        local_logits_sem.masked_fill_(vp_nav_masks.logical_not(), -float('inf'))

        global_fuse_logits = global_logits + global2_logits
        local_logits = local_logits + local_logits_sem

        # fusion
        fused_logits = torch.clone(global_fuse_logits)
        ...
        outs = {
            ...
            'fw_global': fw_global,
            'fw_global_depth': fw_global2,
            'fw_local': fw_local,
            'fw_local_sem': fw_local_sem,
            'gmap_embeds': gmap_embeds,
            'gmap_depth_embeds': gmap_depth_embeds,
            'vp_embeds': vp_embeds,
    ...
    def forward(self, mode, batch, **kwargs):
        ...
        elif mode == 'depth_panorama':
            pano_depth_embeds, pano_masks = self.forward_depth_panorama_per_step(
                batch['view_depth_fts'], batch['obj_img_fts'], batch['loc_fts'],
                batch['nav_types'], batch['view_lens'], batch['obj_lens']
            )
            return pano_depth_embeds, pano_masks

        elif mode == 'sem_panorama':

            pano_sem_embeds, pano_sem_masks = self.forward_sem_panorama_per_step(
                batch['view_sem_fts'], batch['obj_img_fts'], batch['loc_fts'],
                batch['nav_types'], batch['view_lens'], batch['obj_lens']
            )
            return pano_sem_embeds, pano_sem_masks

        elif mode == 'navigation':
            return self.forward_navigation_per_step(
                batch['txt_embeds'], batch['txt_masks'], batch['gmap_img_embeds'], batch['gmap_depth_embeds'],batch['gmap_sem_embeds'],
                batch['gmap_step_ids'], batch['gmap_pos_fts'], batch['gmap_masks'],
                batch['gmap_pair_dists'], batch['gmap_visited_masks'], batch['gmap_vpids'],
                batch['vp_img_embeds'], batch['vp_depth_embeds'], batch['vp_sem_embeds'], batch['vp_pos_fts'], batch['vp_masks'],
